...
def sanitize_env_var(name):
	value = os.getenv("GREEN_API_INSTANCE_ID")
	if value is None:
...
message_classifier_run_every_seconds = sanitize_env_var("MESSAGE_CLASSIFIER_RUN_EVERY_SECONDS")
message_classifier_run_every_seconds = 3000 if message_classifier_run_every_seconds is None else int(message_classifier_run_every_seconds)
