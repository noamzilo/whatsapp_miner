...
class YoJsonFormatter(logging.Formatter):
...
def setup_logger(log_location=None,
	 ...
	 * YO_ENABLE_DEBUG_LOG (bool) - overrides create_debug_log
	 * YO_ENABLE_LOCAL_LOG (bool) - overrides create_fs_log
	 * YO_ENABLE_REMOTE_LOG (bool) - overrides create_remote_log
	 * YO_LOCAL_LOG_SEVERITY (int) - overrides main_log_severity (WARNING - 30, INFO - 20, DEBUG - 10, etc.)
	 * YO_REMOTE_LOG_SEVERITY (int) - remote_log_severity
	 * LOGGING_FORMAT_ENV (str) - console_log_format

	:param log_location: Folder path where to store log files (default - `/yoye/logs/{repo_name}`
	...
	with _log_lock:
		...
		logzio_formatter = YoJsonFormatter(
		...
		create_debug_log = str2bool(os.environ.get(consts.ENABLE_DEBUG_LOG.replace("UV", "YO"), str(create_debug_log)))
		create_fs_log = str2bool(os.environ.get(consts.ENABLE_LOCAL_LOG.replace("UV", "YO"), str(create_fs_log)))
		create_remote_log = str2bool(os.environ.get(consts.ENABLE_REMOTE_LOG.replace("UV", "YO"), str(create_remote_log)))
		main_log_severity = int(os.environ.get(consts.LOCAL_LOG_MIN_SEVERITY.replace("UV", "YO"), str(main_log_severity)))
		remote_log_severity = int(os.environ.get(consts.REMOTE_LOG_MIN_SEVERITY.replace("UV", "YO"), str(remote_log_severity)))
		...
		if create_fs_log:
			if not log_location:
				log_location = f"/yoye/logs/{repo_info.repo_name()}"
